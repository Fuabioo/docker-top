...
pub fn render(
    ...
    header::render(frame, chunks[0], connected, last_poll_secs);

    match view {
        ViewMode::Table => {
            table::render(frame, chunks[1], projects, table_state, columns, sort);
        }
        ViewMode::Chart => {
            chart::render(frame, chunks[1], projects);
        }
    }
