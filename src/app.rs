...
impl App {
    pub fn new(hide_footer: bool) -> Self {
        ...
        Self {
            ...
            error_msg: None,
            should_quit: false,
            last_docker_update: Instant::now(),
    ...
    pub fn last_poll_secs(&self) -> u64 {
        self.last_docker_update.elapsed().as_secs()
    }
    ...
    fn handle_event(&mut self, event: AppEvent) {
        match event {
            ...
            AppEvent::Tick => {
                // Only redraw when the displayed poll counter changes
            ...
            AppEvent::DockerUpdate(projects) => {
                self.projects = projects;
                self.apply_sort();
                self.connected = true;
                self.error_msg = None;
                self.last_docker_update = Instant::now();
                self.last_rendered_poll_secs = 0;
                self.clamp_selection();
                self.dirty = true;
            }
            AppEvent::DockerError(msg) => {
    ...
    fn select_next(&mut self) {
        if !self.projects.is_empty() {
            let i = self.selected();
            let next = (i + 1).min(self.projects.len() - 1);
    ...
    fn select_last(&mut self) {
        if !self.projects.is_empty() {
            self.table_state.select(Some(self.projects.len() - 1));
        }
    }
    ...
    pub fn apply_sort(&mut self) {
        ...
        self.projects.sort_by(|a, b| {
            let ord = match self.sort.column {
                ...
                SortColumn::Ctrs => a.container_count().cmp(&b.container_count()),
                SortColumn::Cpu => a
                    .total_cpu
